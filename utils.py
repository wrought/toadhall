...
##################################################################
# 1/13/2010 fix for sorted since python 2.3.4 lacks it in builtins
...
WEB_ROOT = os.path.dirname(os.path.join(os.getcwd(), __file__))
...
def make_heading(tag, text):
    return '\n<%s id="%s">%s</%s>' % (tag, make_id(text), text, tag)
...
class Date:
    ...
    def __str__(self):
        return '%04d-%02d-%02d' % self.yld()

    def __hash__(self):
        return hash(self.yld())

    def __cmp__(self, other):
        return cmp(self.yld(), other.yld())
    ...
    def __sub__(self, days):
        if isinstance(days, Date):
            return (self.time - days.time)/(24*3600)
        return Date(*time.localtime(self.time - days*24*3600)[:3])
...
def format_toc(text, hpat=re.compile(r'\n *!!?(.*)')):
    toc = [li(link('#' + make_id(heading), esc(heading)))
           for heading in hpat.findall('\n' + text)]
    return toc and div(div(h2('Contents'), c='heading'),
                       div(ul(toc), c='content'), c='toc')
...
#   text = olipat.sub(r'\n<ol><li value="\1">\2</ol>', text)
    ...
    text = ddpat.sub(r'\n<dl><dd>\1</dl>', text)
    text = h2pat.sub(lambda m: make_heading('h2', m.group(1)), text)
    text = h3pat.sub(lambda m: make_heading('h3', m.group(1)), text)
    ...
    if wiki:
        ...
        while 1:
            ...
            if '@' in code:
                # The part before the '@' is the anchor text -- we don't add
                # square brackets because it's not a Kwiki page title.
                anchor, title = code.split('@', 1)
                anchor = anchor.strip()
                title = title.strip()
            else:       
                # If there's no '@', interpret the link as a Kwiki page title;
                # add square brackets as a way of illustrating Kwiki syntax.
                title = code.strip()
                anchor = '[' + title + ']'
            anchor = esc(anchor)
            results += [text[pos:match.start()]]
            if title.startswith('http://') or title.startswith('https://'):
                # If the Kwiki page title looks like a URL, turn it into an
                # external link.
                url = title
                results += ['<a href="%s">%s</a>' % (
                            title.replace('http', 'ht&nbsp;tp'),
                            anchor.replace('http', 'ht&nbsp;tp'))]
            else:
                # Decorate the link appropriately depending on whether the
                # intended page exists, was moved, or never existed.
                ...
                results += ['<a href="%s/%s" class="%s" title="%s">%s</a>' % (
