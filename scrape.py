...
__date__ = '$Date: 2008/07/06 11:54:25 $'.split()[1].replace('/', '-')
...
def fetch(url, data='', agent=None, referrer=None, charset=None, verbose=0,
    ...
    else:  # Non-conformant reply.  Some servers really do this!
        match = re.search('\r?\n[ \t]*\r?\n', reply)
        if match:
            ...
            match = re.search('\r?\n\W', reply)
            if match:
                head, content = head[:match.start()], head[match.end() - 1:]
    head = head.replace('\r\n', '\n').replace('\r', '\n')
...
def getnumber(text):
    """Find and parse an integer or floating-point number in the given text,
       ignoring commas, dollar signs, and percentage signs."""
    text = re.sub(r'[,$%]', '', striptags(text)).strip()
    try:
        return int(text)
    except:
        try:
            ...
            pass
    raise ScrapeError('%r is not a number' % text)
