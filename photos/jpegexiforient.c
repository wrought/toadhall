...
main (int argc, char **argv)
{
  ...
  /* -- modified by Ping 2008-02-22: don't barf on APP0, just skip it. */
  ...
  /* if we see an APP0 JFIF segment, skip it */
  if (exif_data[2] == 0xFF &&
      exif_data[3] == 0xE0) {
    length = read_2_bytes();
    fseek(myfile, length - 2, SEEK_CUR);
    for (i = 2; i < 4; i++)
      exif_data[i] = (unsigned char) read_1_byte();
  }
