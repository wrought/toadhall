...
# Ignore tags other than those we use (make, model, orientation, time, flash).
# This increases the parsing speed about 2x. -- Ping, 2008-02-17
# Start of edit.
    if tag_id not in [0x010f, 0x0110, 0x0112, 0x0132,
                      0x8769, 0x9003, 0x9004, 0x9209]:
        tiff.seek(pos+4)
        return
# End of edit.
    ...
    if value_table:
	    ...
	    val = value_table
        if tag.name not in value_map: # ignore duplicates -- Ping, 2008-02-23
...
def parse(path_name, verbose = 0, mode = 0):
	...
	data = file.read(12)
        ...
        # Add handling of JFIF files with an APP0 segment. -- Ping, 2008-02-22
        if data[0:4] == '\xff\xd8\xff\xe0':
            # JFIF
            length = ord(data[4]) * 256 + ord(data[5])
            file.seek(4, 0)
            file.read(length) # skip APP0 segment
            data = '\xff\xd8' + file.read(10)

	if data[0:4] == '\xff\xd8\xff\xe1' and data[6:10] == 'Exif':
            # JPEG
	    length = ord(data[4]) * 256 + ord(data[5])
	    if verbose > 1:
