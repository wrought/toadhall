...
if form.submit:
    ...
    if form.smoochable not in ['m', 'f', 'mf']:
        quit('Are you looking for frog princes or princesses?')

    sample = {'mf': 59, 'f': 28, 'm': 31}[form.object]
    try:
    ...
    file.write('%d\tmakeout\t%s\t%s\t%s\n' %
               (time.time(), form.subject, form.smoochable, form.makeout))
    file.close()
    print 'Location: index.cgi\n'

else:
    quotients, counts = get_hotness('do', {'m': 31, 'f': 28})
    ...
    quotients, counts = get_hotness('makeout', {'m': 31, 'f': 28})
