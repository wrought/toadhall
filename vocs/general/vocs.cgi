...
members = readlines('members.txt')
managers = readfields('managers.txt')
...
y = 0
def managerform((position, comp, name), options):
    global y
    ...
    form = div('Is ', strong(name), ' doing a good job as ',
               linkposition, '?', buttons, br,
...
if form.submit:
    ...
    for (position, comp, name) in managers:
        ...
        if form.member != 'comment-only':
            if votefield not in form:
                continue
                prologue('Kingman Hall: Votes of Confidence', 'style.css')
                write("You didn't choose YES, NO, or RECALL for %s (%s). "
                      % (name, position))
    ...
    write(p, 'You can only cast YES/NO/RECALL votes once, ',
